...
class Helper {
    ...
    public static function duplicateImage($imagePath, string $folder, string $name): ?string
    {
        // Get the full public path of the original image
        $originalImagePath = public_path($imagePath);
        ...
        // Generate a new name for the duplicated image (e.g., appending 'copy_' to the original name)
        $newImageName = Str::slug($name) . '.' . pathinfo($imagePath, PATHINFO_EXTENSION);

        // Define the new image path
        $newImagePath = public_path('uploads/' . $folder . '/' . $newImageName);
